...
if __name__ == '__main__':  # Esta linha é usada para controle de escopo de execução, de forma que será executado
    # somente o que está dentro deste if, mesmo que seja transformado o código em um arquivo interpretado
    n = int(
        input().strip())  # Esta linha pega o que o usuário irá digitar (comando input), pegar até antes de um espaço
    # ou enter que tiver (strip) e transformar a entrada para um número inteiro e guardar na variável n
    if n % 2 == 1:  # Aqui é usado o comando % para pegar o resto da divisão e, nesta linha, verifica se o número n é
        # ímpar
        print("Weird")  # impresso se número for ímpar
    # A partir daqui, somente será executado caso n seja par
    elif n >= 2 and n <= 5:  # Ao invés de usar else if, como em outras linguagens, o Python tem o elif que é a
        # junção do else e de um novo if
